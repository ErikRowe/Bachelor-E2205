...
#########################################################################################
#   Data reader and simulato
path = 'path' #insert path to file

df = pd.read_csv(path,skiprows=1)
title = ''
# Inital conditions for the simulator
lin_speed_init = [0,0,0]
ang_speed_init = [0,0,0]
rot_init = [0,0,0]
lin_init = [0,0,0]
...
# Start and stop points from datafile, see info.txt to see start and stop points
start = 0
stop = -1
...
# Integral, set everything to 0 do deactivate
Kxi = 0
c_i = 0
windup_linear = 0
windup_angular = 0
...
def Kp(q):
    return Combine4(np.transpose(R.from_quat(q).as_matrix())*Kx, Zero3x3, Zero3x3, c_x*np.diag([2,2,1]))


def Ki(q):
    return Combine4(np.transpose(R.from_quat(q).as_matrix())*Kxi, Zero3x3, Zero3x3, c_i*np.diag([2,2,1]))
...
def f(zeta,t):
    ...
    for i in range(3):
        if abs(integral[i]) > windup_linear:
            integral[i] = sign(integral[i]) * windup_linear
        if abs(integral[i+3] > windup_angular):
            integral[i+3] = sign(integral[i+3]) * windup_angular
    vDot = np.linalg.inv(M)@(-C(v)@v-D(v)@v-Kd@v-Kp(q)@z-integral)
...
x_init = np.array([lin_init[0],lin_init[1],lin_init[2]])
q_init = np.array(get_quaternion_from_euler(rot_init[0],rot_init[1],rot_init[2]))
lin_speed = [lin_speed_init[0],lin_speed_init[1],lin_speed_init[2]]
ang_speed = [ang_speed_init[0],ang_speed_init[1],ang_speed_init[2]]
v_init = np.concatenate((lin_speed, ang_speed))
zeta0 = np.concatenate([x_init, q_init, v_init])

## Desired position and attitude
x_d = np.array([lin_d[0], lin_d[1], lin_d[2]])
q_d = np.array(get_quaternion_from_euler(rot_d[0],rot_d[1],rot_d[2]))
...
axs[0,0].plot(t, eta[0],linestyle='dotted',color = 'g') # Not used for Beluga
...
# fig.legend(['Reference','Measured']) #Beluga
plt.show()