...
void SetpointHolderClass::update_setpoint(Eigen::Vector6d &setpoint_changes, std::vector<bool> buttons,
    ...
    for (int i = 0; i < 3; i++){
        if (setpoint_changes[i]){ //If direction is eligible, perform setpoint change
...
void SetpointHolderClass::handle_button_input(Eigen::Vector3d &setpoint_change_lin, Eigen::Quaterniond &setpoint_change_att,
    ...
    for (auto &action : actions){
        ...
        if(buttons[counter - 1] && !last_tick_active_buttons[counter]){
            action();
            last_tick_active_buttons[counter] = true;
        }
        else if(!buttons[counter - 1] && last_tick_active_buttons[counter]){
